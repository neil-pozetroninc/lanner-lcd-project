...
int main(int argc, char *argv[])
{
	...
	if(argc == 2)
	{
		...
		if(strcmp("-Keypad", argv[1]) == 0)
		{
			...
			do{
				...
				{
					case 0x00:
						snprintf(Keypad_Message, sizeof(Keypad_Message), "Up-");
						break;
					case 0x20:
						snprintf(Keypad_Message, sizeof(Keypad_Message), "Left-");
						break;
					case 0x08:
						snprintf(Keypad_Message, sizeof(Keypad_Message), "Right-");
						break;
					case 0x28:
						snprintf(Keypad_Message, sizeof(Keypad_Message), "Down-");
						break;
				} 	
				switch(detect_press)
				{
					case 0x40:
						{
						size_t len = strlen(Keypad_Message);
						snprintf(Keypad_Message + len, sizeof(Keypad_Message) - len, "Press     ");
					}
						break;
					case 0x00:
						{
						size_t len = strlen(Keypad_Message);
						snprintf(Keypad_Message + len, sizeof(Keypad_Message) - len, "Release     ");
					}
						break;
				}
	...
	snprintf(Keypad_Message, sizeof(Keypad_Message), "Cursor Off:");
	write(devfd, Keypad_Message, strlen(Keypad_Message));
	sleep(2);
	printf("  Display Control - Cursor On\n");
	ioctl(devfd, PLCM_IOCTL_DISPLAY_C, 1);
	snprintf(Keypad_Message, sizeof(Keypad_Message), "Cursor On:");
	write(devfd, Keypad_Message, strlen(Keypad_Message));
	sleep(2);
	printf("  Display Control - Blinking On\n"); 
	ioctl(devfd, PLCM_IOCTL_DISPLAY_B, 1);
	snprintf(Keypad_Message, sizeof(Keypad_Message), "Blinking On:");
	write(devfd, Keypad_Message, strlen(Keypad_Message));
	sleep(2);
	printf("  Display Control - Blinking Off\n"); 
	ioctl(devfd, PLCM_IOCTL_DISPLAY_B, 0);
	snprintf(Keypad_Message, sizeof(Keypad_Message), "Blinking Off:");
	write(devfd, Keypad_Message, strlen(Keypad_Message));
	sleep(2);
	...
	Ret_Message1[sizeof(Ret_Message1) - 1] = '\0';
	printf("  Read: %s\n", Ret_Message1);
	sleep(2);
	ioctl(devfd, PLCM_IOCTL_SET_LINE, 2);
	write(devfd, LCM_Message2, strlen(LCM_Message2));
	...
	Ret_Message2[sizeof(Ret_Message2) - 1] = '\0';
	printf("  Read: %s\n", Ret_Message2);
	sleep(2);
	...
	printf("CGRAM Test...\n");
	ioctl(devfd, PLCM_IOCTL_SET_LINE, 1);
	snprintf(Keypad_Message, sizeof(Keypad_Message), " CGRAM Testing:");
	write(devfd, Keypad_Message, strlen(Keypad_Message));
	ioctl(devfd, PLCM_IOCTL_SET_LINE, 2);
	...
	printf("Keypad Testing....\n");
	ioctl(devfd, PLCM_IOCTL_SET_LINE, 1);
	snprintf(Keypad_Message, sizeof(Keypad_Message), " Keypad Testing");
	write(devfd, Keypad_Message, strlen(Keypad_Message));
	ioctl(devfd, PLCM_IOCTL_SET_LINE, 2);
	snprintf(Keypad_Message, sizeof(Keypad_Message), "  Press the 4 buttons");
	write(devfd, Keypad_Message, strlen(Keypad_Message));
	...
	do{
		...
		if((MaxTestTime%10) == 0){
			ioctl(devfd, PLCM_IOCTL_SET_LINE, 1);
			snprintf(Keypad_Message, sizeof(Keypad_Message), " Keypad Testing....%02d",
				MaxTestTime/10);
			write(devfd, Keypad_Message, strlen(Keypad_Message));
		}
		MaxTestTime--;
		if(MaxTestTime == 0 || Counter==0xff)
		{
			printf("Keypad Testing Finished.\n");
			ioctl(devfd, PLCM_IOCTL_SET_LINE, 1);
			snprintf(Keypad_Message, sizeof(Keypad_Message), " Keypad Test Finished");
			write(devfd, Keypad_Message, strlen(Keypad_Message));
			break;
		}
		usleep(100000); // 100 msec
